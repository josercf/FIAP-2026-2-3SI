...
Serviço congelado das Aulas 02 e 03, **evoluído nesta aula**. Ele não é
tarefa: não edite este arquivo. O que vocês escrevem hoje é o
`docker-compose.yml` da raiz do laboratório.

O que mudou em relação à Aula 03, e por quê
-------------------------------------------

Na Aula 03 o coletor gravava cada posição em `LOGITECH_DADOS`
(`dados/telemetria.jsonl`) e o painel **lia esse mesmo arquivo**. A ADR-002
registrou isso como simplificação deliberada e nomeou o ponto de
substituição: a Aula 07. É hoje.

O coletor passa a ter um **terceiro socket**, agora na camada 7:
  ...
  HTTP 8082/tcp  **novo**: a API de leitura da telemetria
                 GET /health      -> {"status": "ok"}
                 GET /telemetria  -> a última posição conhecida de cada placa

O painel deixa de abrir arquivo e passa a chamar `GET /telemetria`. Os dois
serviços continuam gravando e lendo o mesmo dado, mas agora por um
**contrato de API**, não por um arquivo que os dois precisam enxergar. Quem
manda no endereço é a variável `LOGITECH_TELEMETRIA_URL`, lida pelo painel.

O arquivo em `LOGITECH_DADOS` continua existindo: ele é a persistência do
coletor, não mais o canal de comunicação. É a diferença entre "os dois
serviços compartilham um arquivo" e "um serviço tem um banco e o outro
consulta a API dele".
