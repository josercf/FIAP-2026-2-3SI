# Evidências do laboratório da Aula 11 (execução de referência)

> Os números abaixo são de **uma** execução, feita na construção deste lab
> kit em 31/07/2026, com 12 caminhões no simulador e o atraso padrão de
> 800 ms no serviço de Faturamento. Os seus podem ser diferentes, e é para
> isso que o `verificar.py` confere coerência entre os números em vez de
> comparar com uma tabela fixa.

**Dupla:** execução de referência do professor
**Fork:** o próprio lab kit
...
```
SSE_ASSINANTES_PAINEL_ABERTO: 1
SSE_ASSINANTES_PAINEL_FECHADO: 0
```
