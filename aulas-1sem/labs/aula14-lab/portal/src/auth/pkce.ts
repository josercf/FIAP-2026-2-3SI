 ...
 *
 * ESTE ARQUIVO É SEU. É a lacuna TODO-5 do laboratório.
 *
 ...
 *
 * Dica: `crypto.getRandomValues(new Uint8Array(48))` e depois `base64url`.
...
/**
 * TODO-5b: derive o `code_challenge` do verificador.
 *
 ...
 *
 * Dica: `await crypto.subtle.digest('SHA-256', new TextEncoder().encode(v))`
 * devolve um ArrayBuffer; `new Uint8Array(...)` sobre ele, e `base64url`.
 */
...
/**
 * TODO-5c: monte a URL de autorização.
 *
 ...
 *
 * Use `URLSearchParams` e não concatenação: o `redirect_uri` tem `:` e `/`
...
/**
 * TODO-5d: troque o código de autorização pelo token.
 *
...
export async function trocarCodigoPorToken(
  ...
  throw new Error('TODO-5d ainda nao implementado em portal/src/auth/pkce.ts');
}
