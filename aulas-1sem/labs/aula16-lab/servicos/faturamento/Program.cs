...
//
// 3. Validação de JWT com RBAC (ADR-009), em `Seguranca.cs`. Com
//    LOGITECH_AUTH_ATIVA desligada este serviço se comporta exatamente como
//    na Aula 11.
//
// Rotas (contrato da plataforma, ADR-006, ADR-008 e ADR-009):
//   GET  /health                      aberta, sempre
//   POST /api/v1/faturas              ADMIN
//   GET  /api/v1/faturas/{pedidoId}   ADMIN
//   GET  /api/v1/metricas             instrumentação de laboratório, aberta
//   POST /api/v1/metricas/zerar       instrumentação de laboratório, aberta
//
// Uso:
...
aplicacao.MapGet("/health", () => Results.Ok(new
{
    ...
    banco = bancoUrl,
    atrasoMs,
    authAtiva = Seguranca.Ativa(),
    corsOrigens = origens,
    persistencia = "memória (a versão da Aula 05 usa EF Core sobre o schema faturamento)"
}));

// Emitir fatura é ato de ADMIN (ADR-009). Consequência que a Aula 16 mostra
// em sala: o serviço de Pedidos propaga o token de quem criou o pedido, e um
// usuário com papel CLIENTE recebe 403 aqui. O pedido fica gravado, a fatura
// não sai, e a `jornada` da resposta do POST /api/v1/pedidos diz exatamente
// isso. Não é defeito: é o contrato funcionando.
aplicacao.MapPost("/api/v1/faturas", async (EntradaFatura entrada, HttpContext contexto) =>
{
    var veredito = await Seguranca.ExigirAsync(contexto.Request.Headers.Authorization, "ADMIN");
    if (!veredito.Autorizado)
    {
        return Results.Json(new { erro = "acesso negado", detalhe = veredito.Motivo },
                            statusCode: veredito.Status);
    }
...
aplicacao.MapGet("/api/v1/faturas/{pedidoId:long}", async (long pedidoId, HttpContext contexto,
...
Console.WriteLine($"[AUTH] validação de token: {(Seguranca.Ativa() ? "exigida" : "DESLIGADA")}");
